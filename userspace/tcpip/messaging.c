...
static void handle_open(UserMessage* message){
    ...
    answer.header.source = getpid();
    answer.header.target = message->header.source;
    answer.type = MSG_RESPONSE;
    answer.int_response = result;
...
static void handle_send (UserMessage* message){
    if (message->header.number == 0){
        stack_socket_udp_send(
            message->header.source,
            message->send_header_request.socket_type,
            message->send_header_request.port,
            message->send_header_request.remote_address,
            message->send_header_request.remote_port,
            message->send_header_request.payload,
            message->send_header_request.size,
            0
        );
    } else {
        stack_socket_udp_send(
            message->header.source,
            message->send_request.socket_type,
            message->send_request.port,
            0,
            0,
            message->send_request.payload,
            message->send_request.size,
