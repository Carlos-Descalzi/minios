...
int stack_init (void){
    ...
    eth_fd = open(network_device, O_RDWR | O_NONBLOCK); 
    ...
    ioctl(eth_fd, IOCTL_NET_MAC, device_mac);

    log("Mac address   :");
    print_mac(device_mac);
    log("\n");

    arp_announce(tx_buffer);
    arp_query(gateway, tx_buffer);
...
int stack_loop(){
    ...
    int received = read(eth_fd, rx_buffer, BUFFER_SIZE);
    ...
    if (received > 0){
        ...
        switch(htons2(eth_frame->ether_type)){
            case ETHER_TYPE_ARP:
                log("\tARP packet\n");
                arp_handle_packet(eth_frame, tx_buffer);
                break;
            case ETHER_TYPE_IP:
                log("\tIPv4 packet\n");
                ipv4_handle_packet(eth_frame);
                break;
            default:
...
int stack_socket_udp_send(
        int pid, int socket_type, uint16_t port, 
        uint32_t remote_address, uint16_t remote_port,
        void* data, uint16_t length,
        int commit) {

    if (socket_type & SOCKET_TYPE_TCP){
        log("This socket is not UDP\n");
        return -1;
    }
    return ipv4_udp_socket_send(pid, port, remote_address, remote_port, data, length, commit);
}
